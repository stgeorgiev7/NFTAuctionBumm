...
export default function Avatar({ url, size = 20, verified = false }) {
  const [badgeSize, badgeSizeSet] = useState(size / 2);

  const badgeImg = (
    <img style={{ height: badgeSize }} src={verifiedImg.src}></img>
  );
  ...
  return (
    ...
    >
      <AvatarComp
        src={url ? url : defaultImg.src}
        sx={{ width: size, height: size, margin: "5px 0px 0px"}}
      />
    </Badge>
  );
}
